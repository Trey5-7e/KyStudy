...
impl PlanningRepository for SqlitePlanningRepository {
    ...
    fn set_plan_status(
        ...
        let changed = transaction
            .execute(
                ...
                params![plan_id, status.as_str(), updated_at, workspace_id],
            )
            .map_err(database_error)?;
        if changed == 0 {
    ...
    fn save_stage(&self, stage: PlanStage) -> Result<PlanStage, PlanningError> {
        let connection = self.open()?;
        ensure_plan_exists(&connection, &stage.plan_id)?;
    ...
    fn add_reference(&self, mut reference: PlanReference) -> Result<PlanReference, PlanningError> {
        let connection = self.open()?;
        ensure_plan_exists(&connection, &reference.plan_id)?;
...
fn ensure_plan_exists(connection: &Connection, plan_id: &str) -> Result<(), PlanningError> {
    let exists = connection
        .query_row(
            "SELECT EXISTS(SELECT 1 FROM study_plan WHERE id = ?1)",
            params![plan_id],
...
mod tests {
    ...
    use crate::application::{
        AddPlanReferenceInput, PlanningUseCases, ResourceUseCases, SavePlanInput,
