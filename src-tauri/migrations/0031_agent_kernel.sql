CREATE TABLE ai_agent_scope (
    id TEXT PRIMARY KEY,
    workspace_id TEXT NOT NULL REFERENCES workspace(id) ON DELETE CASCADE,
    schema_version INTEGER NOT NULL DEFAULT 1 CHECK(schema_version = 1),
    grant_json TEXT NOT NULL CHECK(json_valid(grant_json) AND length(grant_json) <= 65536),
...
CREATE TABLE ai_agent_run (
    ...
    input_limit INTEGER NOT NULL CHECK(input_limit BETWEEN 1 AND 48000),
    output_limit INTEGER NOT NULL CHECK(output_limit BETWEEN 1 AND 8000),
...
CREATE TRIGGER agent_run_immutable BEFORE UPDATE ON ai_agent_run BEGIN
    SELECT CASE WHEN NEW.id!=OLD.id OR NEW.workspace_id!=OLD.workspace_id OR NEW.conversation_id!=OLD.conversation_id OR NEW.scope_id!=OLD.scope_id
        OR NEW.model_limit!=OLD.model_limit OR NEW.tool_limit!=OLD.tool_limit OR NEW.input_limit!=OLD.input_limit OR NEW.output_limit!=OLD.output_limit OR NEW.active_limit_ms!=OLD.active_limit_ms
...
CREATE TABLE ai_agent_step (
    ...
    result_json TEXT CHECK(result_json IS NULL OR (json_valid(result_json) AND length(result_json) <= 16384)),
...
CREATE TRIGGER agent_scope_immutable BEFORE UPDATE ON ai_agent_scope BEGIN
    SELECT CASE WHEN NEW.id!=OLD.id OR NEW.workspace_id!=OLD.workspace_id OR NEW.grant_json!=OLD.grant_json OR NEW.schema_version!=OLD.schema_version
        OR (OLD.revoked_at IS NOT NULL AND NEW.revoked_at IS NOT OLD.revoked_at)
        THEN RAISE(ABORT,'agent_scope_immutable') END;
END;
