...
| 对应 PRD   | 0.1                                    |
| 更新日期   | 2026-07-22                             |
| 状态       | 概念模型；M1-M9 子集已落地至 schema v12 |
| 数据库方向 | SQLite，本地单工作区优先               |

## 1. 设计目标

> 实现说明：schema v12 已落地多个 Provider、规划对话与消息、调用到 PDF 页码的上下文引用，以及 AI 回复到待确认计划草案的来源关系；每个 API Key 只保存到系统安全凭据存储，不进入 SQLite 或普通备份。OCR 和派生题目识别仍按本文概念边界后续实现。
