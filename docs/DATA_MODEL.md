...
| 对应 PRD   | 0.1                               |
| 更新日期   | 2026-07-22                        |
| 状态       | 概念模型；当前已落地至 schema v19 |
| 数据库方向 | SQLite，本地单工作区优先          |

## 1. 设计目标

> 实现说明：schema v14 已落地多个 Provider、规划对话与消息、调用到 PDF 页码的上下文引用、AI 回复到待确认计划草案的来源关系、题目区域的 OCR 草稿/确认历史/逐行文字框，以及计划阶段确认展开到日程的稳定来源关系；每个 API Key 只保存到系统安全凭据存储，不进入 SQLite 或普通备份。自动切题、图片导图识别和派生题目候选仍按本文概念边界后续实现。
...
| 待确认草案   | 规划、导图识别、题目检测和 OCR 草稿   | 默认包含，可单独清理失败草案 |
| 派生索引     | PDF 文本片段、全文索引、向量          | 可重建，可选择不备份         |
| 用户确认内容 | OCR 确认文本、个人纠正与正式关联      | 必须备份，不能静默重建       |
...
| `source_format`             | `opml`、`freemind` 或 `xmind`       |
| `title`                     | 草案建议标题                        |
...
AI 草案任务在确认前不进入 `task`。schema v14 的阶段展开由后端根据用户确认的日期范围与星期规则生成任务，前端不能直接提交候选任务数组。

### 9.4 `plan_stage_task`

schema v14 使用独立来源表记录“某个阶段在哪个原始日期生成了哪项正式任务”，避免把计划来源字段继续塞入已经稳定的 `task` 表。

| 字段             | 说明                                                   |
| ---------------- | ------------------------------------------------------ |
| `task_id`        | 正式任务，主键；任务永久删除时级联移除来源记录         |
| `stage_id`       | 原计划阶段；阶段删除时置空，已经确认的正式任务继续保留 |
| `generated_date` | 首次生成时的原始日期；任务延期后保持不变               |
| `created_at`     | 确认写入时间                                           |

只要阶段仍存在，`stage_id + generated_date` 必须唯一。重复确认、任务延期、取消、恢复或进入回收站都不得让同一阶段的同一原始日期再次生成任务；这条来源关系不等同于任务当前日期，也不随延期更新。

### 9.5 计划执行进度派生视图

M13 不新增统计表或快照。每次进入规划页时，以 `plan_stage_task` 为来源集合，联结 `task` 当前状态和 `study_session` 真实记录后聚合：

- `generated = effective + canceled + trashed`；
- `effective = completed + remaining`，其中 remaining 只包括 `todo` 和 `in_progress`；
- 完成率为 `completed / effective`，effective 为 0 时显示“—”；
- 逾期是 remaining 中当前 `planned_date` 早于工作区今日日期的任务，延期后按新日期判断；
- 计划分钟只汇总 effective 任务的 `estimated_minutes`；
- 实际分钟汇总来源任务全部未删除的 `study_session`，任务后来取消或回收不抹掉真实投入；
- 阶段删除后来源关系的 `stage_id` 已置空，因此不再归入任何现存阶段或计划。

Command DTO 只返回上述计数、分钟、百分比和阶段标识，不返回任务 ID 列表、SQL、内部来源行或学习记录明细。
...
### 10.4 `question_region_ocr`

schema v13 为每个正式题目区域保存 OCR 草稿和用户确认历史，不在该表复制题目、文档或页码；这些归属通过 `region_id` 回溯，避免区域移动或删除后产生悬空副本。

| 字段                        | 说明                               |
| --------------------------- | ---------------------------------- |
| `id`                        | 识别记录 ID                        |
| `region_id`                 | 来源题目区域                       |
| `engine`                    | 经过校验的 OCR 引擎版本            |
| `recognized_text`           | Worker 原始识别文本                |
| `confirmed_text`            | 用户确认文本；草稿时为空           |
| `mean_confidence`           | 0～1 平均置信度                    |
| `state`                     | `draft`、`confirmed`、`superseded` |
| `created_at` / `updated_at` | 创建和更新时间                     |

同一区域最多一份 `draft` 和一份当前 `confirmed`。确认草稿时，旧确认记录在同一事务内转为 `superseded`；重新识别只替换草稿，不提前覆盖确认文本。

### 10.5 `question_region_ocr_line`

| 字段                           | 说明                                   |
| ------------------------------ | -------------------------------------- |
| `id`                           | 文字行 ID                              |
| `recognition_id`               | 所属识别记录                           |
| `text`                         | 单行文字                               |
| `confidence`                   | 0～1 置信度                            |
| `x` / `y` / `width` / `height` | 相对于提交区域图片的 0～1 归一化文字框 |
| `sort_order`                   | Worker 返回的稳定阅读顺序              |

文字框用于后续检索和核对，不代表公式、矩阵或复杂表格的可靠结构。删除识别记录或来源区域时，文字行通过外键级联删除。

### 10.6 `question_attempt`
...
| `enabled`       | 是否为当前 Provider            |
| `deleted_at`    | 可空软删除时间                 |

schema v11 允许一个工作区保存多个 Provider，但所有未删除记录中必须恰好有一个当前配置。删除当前配置时会在同一数据库流程中启用一个剩余配置；没有剩余配置时创建默认离线 Provider。远程配置使用各自独立的 `secret_ref`，软删除不会破坏既有 `ai_call` 历史。
...
schema v12 首批保存本地规划对话与用户/助手消息。每次调用只选取最近 8 条消息并施加总字符上限；尚未实现长期摘要。助手消息通过 `ai_call_id` 关联实际调用，前端 DTO 不暴露调用 ID。
...
schema v12 的 `ai_context_ref` 保存 `document_id + page_number`、引用标签、当时片段哈希和 Token 估算，不重复保存完整 PDF 片段。用户从搜索结果中最多选择 6 页；后端重新读取受控文字片段，前端不能提交任意本地文件内容冒充资料上下文。
...
## 14. 科目题库与 PDF 索引（Schema v17–v19）

### 14.1 `workbook_category`

用户创建的练习册逻辑分类，例如 880、1000 题。它不等同于物理 PDF；同一个分类可以通过多个科目分段引用。

### 14.2 `workbook_document_segment`

把一个 PDF 的连续页面范围映射到一个科目和练习册。混合 PDF 因而可以拆成“线性代数 / 880”和“概率论与数理统计 / 880”，同时继续共享同一份 Blob。

### 14.3 `question_index_metadata`

保存自动索引题目的练习册、PDF 分段、稳定来源键、基础/综合/拓展、识别置信度和原书顺序。题目正文区域继续复用 `question_region`，跨页题由多条按顺序排列的区域组成。

### 14.4 作答状态

“未做”由不存在 `question_attempt` 派生；“做对、做错、不全对”取最近一次不可变 Attempt。历史做错和不全对次数从全部 Attempt 统计，不保存可漂移的汇总标签。持久层沿用兼容值 `uncertain` 表示产品语言中的“不全对”。
...
11. 一个 QuestionRegion 最多有一个 OCR 草稿和一个当前确认结果；重新识别不得覆盖确认文本。
12. OCR 路径、Worker stderr 和第三方原始错误不得进入 WebView DTO 或持久化业务字段。
13. 只有当前 `active` 计划的阶段可以确认生成正式任务，且必须先返回后端生成的精确日期预览。
14. 同一计划阶段的同一原始日期最多生成一个正式任务；延期、取消或回收不得破坏该幂等关系。
15. 删除计划阶段不删除已经确认的任务，只把 `plan_stage_task.stage_id` 置空。
16. 计划执行进度必须由正式来源任务和学习记录即时派生，不保存可漂移的百分比快照；取消和回收不得计入有效任务分母。
17. 题库根层级必须是科目，其次才是练习册；一个 PDF 可以被多个科目分段引用，但不得复制底层 Blob。

### Schema v18：资料安全移除

`resource_document.deleted_at` 表示资料是否从活动资料库移除。活动列表与资料搜索只读取 `deleted_at IS NULL`；题目卡片、历史计划等已经建立的引用仍可读取受管理文件，避免删除资料后丢失题目原图。

schema v18 同时保证：同一 PDF 分段内的自动索引题目由稳定 `source_key` 唯一标识，重建索引不得重复创建题目或覆盖作答历史；当前做题状态必须从最近一次 Attempt 派生，历史做错和不全对次数不得因后来做对而丢失。

### Schema v19：题目缺漏确认

`question_gap_acknowledgement` 只保存用户对题号诊断异常的确认状态，不保存题目正文、PDF 内容或可重建的诊断结果。表结构如下：

| 字段              | 约束与说明                            |
| ----------------- | ------------------------------------- |
| `workspace_id`    | `TEXT NOT NULL`，引用 `workspace(id)` |
| `issue_key`       | `TEXT NOT NULL`，长度 1～500          |
| `acknowledged_at` | `INTEGER NOT NULL`，确认或更新时间    |

表使用 `PRIMARY KEY (workspace_id, issue_key)` 复合主键，并声明为 SQLite `STRICT` 表。`workspace_id` 外键为 `ON DELETE CASCADE`：删除工作区时一并删除确认记录；题目、分段或 PDF 被删除时不会因为 issue key 没有对应外键而级联删除。应用层会先 trim `issue_key`，只接受 `duplicate|`、`jump|` 和 `non_numeric|` 前缀；确认 `false` 通过删除该行恢复提示，确认 `true` 以工作区和 issue key 幂等 upsert 时间。

诊断指纹必须包含能区分异常的稳定内容：

- `duplicate` 和 `jump` 同时包含前后题目 ID 与两侧解析后的数字题号；
- `non_numeric` 包含题目 ID 与题号的原始 `trim()` 文本，不做语义改写；
- `missing` 指纹只用于补题提示，不允许写入确认表，因此缺题不能被“确认无需处理”。

默认诊断只统计未出现在确认表的异常；界面可切换“显示已确认”，并对已确认的 `duplicate`、`jump`、`non_numeric` 执行“恢复提示”。题号变化会改变包含该题号的指纹，旧确认不会压制新的异常；确认状态不跨题目或题号复用。

这张表属于正式的用户确认内容，完整工作区备份必须包含数据库中的确认行，恢复后继续由 schema v19 数据库提供；它不额外包含 PDF、Blob 或临时文件。可重建的 PDF 文本/全文索引缓存仍按通用备份边界处理，不能以重建缓存替代确认记录。
...
- `plan_stage_task(stage_id, generated_date)` 部分唯一，排除已经置空的阶段来源；
...
- `question_region_ocr(region_id, state, updated_at)`，并以部分唯一索引约束当前草稿和确认结果；
...
5. TV-07 与 M10 已验证普通中文 OCR、归一化文字框、区域图片保留和人工确认链路；真实教材、双栏、手写体、复杂公式效果及组件安装体验仍需长期样本验证；
