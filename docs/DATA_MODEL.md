...
| 对应 PRD   | 0.1                                    |
| 更新日期   | 2026-07-22                             |
| 状态       | 概念模型；M1-M8.1 子集已落地至 schema v11 |
| 数据库方向 | SQLite，本地单工作区优先               |

## 1. 设计目标

> 实现说明：schema v11 已落地多个非敏感 Provider/模型配置、软删除、显式启用、Token 预算、调用状态、实际用量和响应缓存；每个 API Key 只保存到系统安全凭据存储，不进入 SQLite 或普通备份。AI 对话、资料上下文引用、OCR 和派生题目识别仍按本文概念边界后续实现。
