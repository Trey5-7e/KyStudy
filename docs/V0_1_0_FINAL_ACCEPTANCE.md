...
- [M13 计划执行进度](archive/v0.1.0/M13_ACCEPTANCE.md)
- [R40 资料驱动计划窗口](archive/v0.1.0/POST_REDESIGN_REVIEW.md)
- [R23 题目缺漏确认](archive/v0.1.0/R23_QUESTION_GAP_ACKNOWLEDGEMENTS_ACCEPTANCE.md)
- [R25 扫描练习册 OCR](archive/v0.1.0/R25_SCANNED_WORKBOOK_OCR_ACCEPTANCE.md)
- [R26 1000 题导入与诊断](archive/v0.1.0/R26_1000_QUESTION_IMPORT_AND_DIAGNOSIS_ACCEPTANCE.md)
- [R32 题库主页与工具窗口](archive/v0.1.0/R32_QUESTION_BANK_HOME_AND_DIALOG_REFINEMENT_ACCEPTANCE.md)
- [R39 页面表面与 AI 设置窗口](archive/v0.1.0/R39_PAGE_SURFACE_AND_AI_SETTINGS_WINDOW_ACCEPTANCE.md)
- [R48 周期计划事项跳过](archive/v0.1.0/R48_CYCLE_PLAN_ITEM_SKIP_ACCEPTANCE.md)
...
## 8. 2026-08-15 R50 本地 OCR 组件管理验收追加记录

项目负责人已确认 R50 本地 OCR 组件管理验收通过，覆盖缺失组件状态、完整安装、修复与失败保护、移除与数据保留、并发与窄窗口响应式场景。结合 R52/R53 的 OCR 公开资产与在线下载构建条件已闭合，当前剩余工作仅为完整 v0.1.0 应用发行包（NSIS/便携版）的最终桌面安装与发布验收。

## 9. 2026-08-15 题库历史题型兼容修复追加记录

- 定位到一条历史题目记录的 `question_type` 为 `NULL`，读取器原先按非空字符串解析，导致整份题库快照被拒绝并显示“题库读取失败”。数据库完整性、迁移版本和题目数据均未损坏。
