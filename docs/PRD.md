...
M6 已落地 REVIEW-001～REVIEW-007：错误作答和手动操作均可激活错题；每日配额与提前补足可配置；队列保存稳定快照和结构化入选原因；四类反馈进入 policy version 1 的不可变复习事件。REVIEW-008 的自定义间隔编辑保留到 M8，避免在缺少长期使用反馈时过早固化配置界面。
