...
| PLAN-008 | 已确认的阶段任务应显示真实完成率、逾期和投入对比     | P1     |
...
M4 已落地 MAP-001、MAP-002、MAP-004，以及 MAP-003 中的节点笔记和资料/PDF 页码关联。MAP-005 首批确定为 OPML 与 FreeMind `.mm`；XMind 原文件可上传，但需先导出为 OPML。任务、题目、错题关联、图片识图、AI 差异和通用导出仍保留在后续批次。
...
M5 已落地 BOOK-001～BOOK-003、BOOK-007，以及 BOOK-008 中的个人解析和多区域来源。框选坐标按 `coordinate_version=1` 保存为 PDF 页面归一化矩形。M10 已为这些正式区域接入按需本地 OCR、可编辑草稿和确认历史；自动题目检测、答案区域语义、AI 标签、批注和附件仍保留在后续批次。
...
M6 已落地 REVIEW-001～REVIEW-007 并完成综合验收：错误作答和手动操作均可激活错题；每日配额与提前补足可配置；队列保存稳定快照和结构化入选原因；四类反馈进入 policy version 1 的不可变复习事件。REVIEW-008 的自定义间隔编辑保留到后续整合版本，避免在缺少长期使用反馈时过早固化配置界面。
...
| AI-010 | 支持多个本地 Provider 配置并显式选择当前配置      | P0     |
| AI-011 | 每个远程 Provider 使用独立系统密钥并可安全删除    | P0     |

M8.1 已落地 AI-001～AI-004、AI-006、AI-009～AI-011 的基础版本。Provider 管理仅支持离线测试与 OpenAI-compatible Responses API；最多 20 个配置，共享工作区 Token 预算，不包含自动路由、故障转移或余额脚本。

M9 首批落地 PLAN-004～PLAN-008 与 AI-005 的受控子集：用户从本地搜索结果中明确选择最多 6 个 PDF 页码，后端重新解析受限片段并结合最近 8 条消息生成完整外发预览；执行前预览内容必须保持一致。回复、调用和页码来源保存到本地，只有用户单独点击后才复制为 `draft` 计划，不自动生成阶段或日程任务。

M11 首批学习分析已经落地：按最近 7/28/90 天聚合正式任务、学习记录、真实作答、每日复习队列和 AI 实际用量，并与紧邻的上一周期比较；同时展示当前积压、科目投入、知识节点薄弱点和反复出错题。该页面完全在本地计算，不调用 AI，也不把尚未建立的计划与任务自动关联包装成计划进度。

M12 已落地 PLAN-006：只有当前 `active` 计划的阶段可以展开到日程。用户明确选择科目、阶段内日期范围、执行星期、任务内容、预计时长和优先级；后端生成精确日期预览并标记已经由该阶段生成的日期，用户再次确认后才原子写入正式任务。重复确认不会重复创建，任务延期、取消或进入回收站后也不会因再次展开而被意外补回。本流程完全在本地执行，不调用 AI 或消耗 Token。

M13 已落地 PLAN-008：规划页按计划和阶段汇总 M12 明确确认的来源任务。完成率只使用未取消、未回收的有效任务；逾期使用延期后的当前任务日期；计划投入只汇总有效任务的预计分钟，实际投入保留所有未删除学习记录的真实历史，即使任务后来取消或进入回收站也不抹除。手工任务、AI 草稿和已经删除的阶段不会被包装成计划进度。本流程为 schema v14 上的本地只读聚合，不产生统计快照、网络请求或 Token 消耗。
...
M8.1 已落地固定侧边导航。“今日”是默认入口，合法的上次访问页会恢复，窄窗口改为横向菜单；AI 对话仍属于具体业务场景，不单独做一个无上下文的万能聊天页。
...
- 用户可以把 OPML 或 FreeMind `.mm` 转换为独立草案，确认前不修改正式导图；
- 用户可以撤销、重做节点编辑和移动，并把节点关联回本地 PDF 页码；
- 图片识图和 AI 修改某个子树时的逐项差异仍属于后续验收场景。
...
4. **M3：资料与规划**——PDF/图片受控阅读、阅读进度、手动个人计划和页码引用；
5. **M4：知识结构**——思维导图编辑、结构化导入草案、节点资料关联和持久化撤销/重做；
6. **M5：PDF 习题册**——手动区域框选、题目元数据、知识节点关联和多次作答记录；
7. **M6：错题复习**——优先级、每日队列、间隔规则和积压分析；
8. **M7：本地检索**——PDF 文字层解析、可恢复分片、FTS5 搜索和页码引用；
9. **M8：AI 基础设施**——Provider、密钥、上下文预览、缓存、预算和用量记录；
10. **M9：AI 规划闭环**——带来源引用的规划对话、版本差异和人工确认写入；
11. **M10：OCR 与识别**——按需 OCR、导图草案和题目区域候选；
12. **M11：整合优化**——首批已落地学习分析；后续继续处理批量操作、性能和长期体验问题；
13. **M12：规划到日程**——阶段日期规则、任务预览、冲突提示、人工确认和来源幂等；
14. **M13：计划执行进度**——阶段完成率、待完成与逾期、计划和实际投入以及状态调整；
15. **M14（可选）：发布准备**——跨平台打包、签名、文档和开源协作规范。
...
1. 已决策：首批正式支持 OPML 与 FreeMind `.mm`；XMind 暂不直接解压，先导出为 OPML；后续再基于真实多版本样本决定兼容范围；
2. 已决策：Windows 首批 OCR 默认使用按需启动的可选本地 sidecar，模型随组件提供且不
   静默调用外部服务；外部 OCR 将来只能作为用户显式选择的同一 Port 适配器；
